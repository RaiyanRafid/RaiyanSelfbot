    ...
    async def nuke(self, ctx):
        ...
        if old_ch:
            ...
            if category:
                ...
                await old_ch.delete()
                new_ch = await category.create_text_channel(old_ch.name, position=position)
                await new_ch.send(f'Nuked by {ctx.author.name}.')
            else:
                await ctx.send("Unable to find category for the old channel.")
        else:
            await ctx.send("Unable to find the old channel.")
